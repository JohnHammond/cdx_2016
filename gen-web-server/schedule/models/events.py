...
class Event(with_metaclass(ModelBase, *get_model_bases())):
    ...
    def get_absolute_url(self):
        return reverse(name_space+'event', args=[self.id])
...
class Occurrence(with_metaclass(ModelBase, *get_model_bases())):
    ...
    def get_absolute_url(self):
        if self.pk is not None:
            return reverse(name_space+'occurrence', kwargs={'occurrence_id': self.pk,
                                                 'event_id': self.event.id})
            return reverse(name_space+'occurrence_by_date', kwargs={
            'event_id': self.event.id,
    ...
    def get_cancel_url(self):
        if self.pk is not None:
            return reverse(name_space+'cancel_occurrence', kwargs={'occurrence_id': self.pk,
                                                        'event_id': self.event.id})
        return reverse(name_space+'cancel_occurrence_by_date', kwargs={
            'event_id': self.event.id,
            'year': self.start.year,
            'month': self.start.month,
            'day': self.start.day,
    ...
    def get_edit_url(self):
        if self.pk is not None:
            return reverse(name_space+'edit_occurrence', kwargs={'occurrence_id': self.pk,
                                                      'event_id': self.event.id})
        return reverse(name_space+'edit_occurrence_by_date', kwargs={
            'event_id': self.event.id,
            'year': self.start.year,
            'month': self.start.month,
            'day': self.start.day,
